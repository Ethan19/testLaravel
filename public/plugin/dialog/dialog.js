...
function dialogSubmit(form){
	...
	url += "&height=500&width=600&is_ajax=1";
 ...
 * @param type string 弹窗控制器(SelectApp)的act
 * @param param string 附加参数
 */
function showSelectDialog(title,type,param,size){
	var dialog_width;
	var dialog_height;
	if(size){
		dialog_width = size.width? size.width:600;
		dialog_height = size.height ? size.height:500;
	}else{
		dialog_width = 600;
		dialog_height= 500;
	}
	href= "?app=select&act="+type+"&width="+dialog_width+"&height="+dialog_height+"&is_ajax=1";
	if(param){
		href += "&param="+param;
	}
...
function selectCurrent(obj){
	var parent_obj = $(obj).attr('parent-obj'),
		param = $('#param').val();
	if(param){
		$('.'+param+' [name='+parent_obj+'_id]').val($(obj).attr("data-id"));
		$('.'+param+' [name='+parent_obj+'_name]').val($(obj).text());
	}else{
		$('[name='+parent_obj+'_id]').val($(obj).attr("data-id"));
		$('[name='+parent_obj+'_name]').val($(obj).text());
	}
	$.dialog.dialogClose();
}
