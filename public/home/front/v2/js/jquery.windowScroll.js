			...
			function scrollStage(obj, stIndex, dir) {
				...
				function move() {
					$(windowobject).animate({
						...
						markStage();
						$(list).eq(sIndex).addClass('active').siblings().removeClass('active');
						$('.side_box').show().css('top',target+776+'px');
					});
				}
			...
			function scrollPage(obj, pageIndex, dir) {
				...
				function move() {
					$(windowobject).animate({
						...
						function() {
							markPage();
							$(list).eq(pageIndex).addClass('selected').siblings().removeClass('selected');
